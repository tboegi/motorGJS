...
typedef enum eProtAscAdsGjs {
  protAscAdsGjsAsc, /* ASCII */
  protAscAdsGjsAds, /* ADS */
  protAscAdsGjsGjs, /* gsi jena steuerung */
} eProtAscAdsGjs;
...
void add_client_con(int fd, int is_listen, eProtAscAdsGjs protAscAdsGjs)
{
  unsigned int i;
  for (i=0; i < NUM_CLIENT_CONS; i++) {
    if (client_cons[i].fd < 0) {
      ...
      client_cons[i].protAscAdsGjs = protAscAdsGjs;
      LOGINFO7("%s/%s:%d add i=%d fd=%d is_listen=%d protAscAdsGjs=%d\n",
               __FILE__,__FUNCTION__, __LINE__,
               i, fd, is_listen, (int)protAscAdsGjs);
      return;
    }
  }
...
static void handle_data_on_GJS_socket(int i, int fd, ssize_t read_res, size_t len_used)
{
  size_t ret;
  ret = handle_gjs_request(fd, &client_cons[i].buffer[0], len_used, CLIENT_CONS_BUFLEN);
  if (ret != len_used) {
        ...
        for (i=0; i < NUM_CLIENT_CONS; i++) {
          ...
          if (FD_ISSET (fd, &rfds)) {
            ...
            if (client_cons[i].is_listen) {
              ...
              add_client_con(accepted_socket, is_listen, client_cons[i].protAscAdsGjs);
              ...
              if (read_res <= 0)  {
                ...
                switch (client_cons[i].protAscAdsGjs) {
                case protAscAdsGjsAsc:
                  handle_data_on_ASC_socket(i, fd, read_res, len_used);
                  break;
                case protAscAdsGjsAds:
                  handle_data_on_ADS_socket(i, fd, read_res, len_used);
                  break;
                case protAscAdsGjsGjs:
                  handle_data_on_GJS_socket(i, fd, read_res, len_used);
                  break;
                default:
                  close_and_remove_client_con_i(i);
                  break;
...
void socket_loop(void)
{
  ...
  add_client_con(listen_socket, is_listen, protAscAdsGjsAsc);
  ...
  add_client_con(listen_socket, is_listen, protAscAdsGjsAds);
  ...
  if (listen_socket < 0)
  {
    LOGERR_ERRNO("no listening socket for GJS !\n");
    exit(3);
  }
  add_client_con(listen_socket, is_listen, protAscAdsGjsGjs);
